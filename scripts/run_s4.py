...
models={"RS_only":["RS"],"RS_Momentum":["RS","Momentum"],"No_Trend":["Momentum","RS","Extension"]}
rows=[]
for name,fs in models.items():
 s["score"]=sum(s[f] for f in fs)/len(fs)
 for split,mask in [("development_2016_2022",(s.Date>="2016-01-01")&(s.Date<"2023-01-01")),("holdout_2023_2026",(s.Date>="2023-01-01")&(s.Date<"2027-01-01"))]:
  x=s.loc[mask].dropna(subset=["f126"]).copy()
  ...
  rows.append({"model":name,"split":split,"months":int(len(z)),"top_mean_return":float(z.top.mean()),"mean_excess":float((z.top-z.u).mean()),"median_excess":float((z.top-z.u).median()),"positive_months":int(((z.top-z.u)>0).sum()),"mean_ic":float(ic.mean()),"median_ic":float(ic.median())})
out=pd.DataFrame(rows);out.to_csv("outputs/s4_holdout_summary.csv",index=False)
hold=out[out.split.str.startswith("holdout")].sort_values("mean_excess",ascending=False)
report={"design":"Frozen candidates selected before holdout evaluation; development=2016-2022, holdout=2023-2026","holdout_results":hold.to_dict(orient="records")}
open("outputs/s4_report.json","w").write(json.dumps(report,indent=2));print(out.to_string(index=False));print(json.dumps(report,indent=2))