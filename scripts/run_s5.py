...
s=s[(s.Date>="2023-01-01")&(s.Date<"2027-01-01")]
...
# leave-one-ticker-out impact on pooled monthly top return
...
# subperiod stability
top["period"]=pd.cut(top.Date,["2022-12-31","2023-12-31","2024-12-31","2025-12-31","2026-12-31"],labels=["2023","2024","2025","2026"])
sub=top.groupby("period",observed=True).f126.agg(["mean","median","count"]).reset_index();sub["universe_mean"]=s.groupby(pd.cut(s.Date,["2022-12-31","2023-12-31","2024-12-31","2025-12-31","2026-12-31"],labels=["2023","2024","2025","2026"]),observed=True).f126.mean().values;sub["excess"]=sub["mean"]-sub["universe_mean"];sub.to_csv("outputs/s5_subperiods.csv",index=False)
